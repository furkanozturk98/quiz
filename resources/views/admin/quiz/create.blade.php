        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('quizzes.store') }}" method="POST">
                    ...
                    <div class="form-group">
                        <input type="checkbox" @if($errors->has('finished_at')) checked @endif onchange="change()">
                        <label>Will there be an end date?</label>
                    </div>

                    <div id = "finished" class="form-group" @if(!$errors->has('finished_at')) style="display: none" @endif>
                        <label>End Date</label>
                        <input type="datetime-local" name="finished_at" class="form-control {{ $errors->has('finished_at') ? 'is-invalid' : '' }}" value="{{ old('finished_at') }}">
